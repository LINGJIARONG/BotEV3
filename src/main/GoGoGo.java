...
public class GoGoGo {
	...
	public static final long ms2turnEmpty = 700; //ms to turn while empty
	...
	public class EV3Helper {
		...
		//WHITE
		private  final float WHITE_R_MIN= 0.13f;
		private  final float WHITE_R_MAX=0.19f;
		private  final float WHITE_G_MIN=0.13f;
		private  final float WHITE_G_MAX=0.19f;
		private  final float WHITE_B_MIN=0.09f;
 ...
 */
	...
	public static void main(String[] args) throws InterruptedException {
		...
		int setHome=0;
		EV3Helper ev3=new GoGoGo().new EV3Helper();
		...
		try {
			while(true) {//to find bricks : just turn left forever 
				...
				if(dis>=0.7||dis<=0.25) //not in range -> turn left to search brick
					ev3.turnLeft(); // initial start time to locate direction
				System.out.println("distance : "+dis); 
				
				if(dis<0.7&&dis>0.25) {//consider as a brick 
