...
public class GoGoGo2 {
	...
	public static final long ms2turn = 1100; // ms to turn while in charge 
	public static final long ms2turnEmpty = 500; //ms to turn while empty
	...
	public class EV3Helper {
		...
		private  final int DEFAULT_MOTOR_SPEED = 888;
		 ...
		 * To be calibrated according to surface.
		 */
		public void turnLeft180(long defau) {
			long time=System.currentTimeMillis(); //start time 
			motorLeft.stop(true);
			motorRight.setSpeed(3000);//todo!!!!!!!!!!
			motorRight.forward();
			long time2=System.currentTimeMillis();
			while( ( time2-time)<=ms2turn-defau) {//todo!!!!!!!!!!
	 ...
	 */
	static long pos;
	static long recul=390;//backward time to leave object alone

	public static void main(String[] args) throws InterruptedException {
		//to skip first return home function()
		int setHome=0;
		EV3Helper ev3=new GoGoGo2().new EV3Helper();
		...
		try {
			while(true) {//to find bricks : just turn left forever 
				...
				//search mode 
				if(dis>=1||dis<=0.3) { //not in range -> turn left to search brick
				...
				if(dis<0.5&&dis>0.3) {//consider as a brick 
					...
					while(ev3.getLastTouch()[0]!=1) {
						...
						if(dis<=0.28) {//wall or a robot dectected
							Thread.sleep(1000);//wait a little a robot pass
							ev3.updateSensors();
							dis=ev3.getLastRange()[0];
							if(dis>=0.28) {//it pass
								obj=false;
	 ...
	 * @throws InterruptedException
	 */
	private static void goHome(EV3Helper e,long de) throws InterruptedException {
		e.turnLeft180(de);
		e.stop();
		e.forward();
